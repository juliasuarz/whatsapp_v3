...
echo "<div class='lista-pendeintes'>";

    echo "<h2>Solicitudes Pendientes</h2>";

    // Consulta SQL para obtener las solicitudes pendientes del usuario actual
    $sqlSolicitudesPendientes = "SELECT tbl_usuarios.id_user, tbl_usuarios.nombre, tbl_usuarios.correo
                                FROM tbl_usuarios
                                INNER JOIN tbl_solicitudesAmistad ON tbl_usuarios.id_user = tbl_solicitudesAmistad.id_emisor
                                WHERE tbl_solicitudesAmistad.id_receptor = $id_usuario_actual
                                AND tbl_solicitudesAmistad.estado = 'pendiente'
                                AND tbl_usuarios.id_user != $id_usuario_actual";

    $resultSolicitudesPendientes = mysqli_query($conn, $sqlSolicitudesPendientes);
    ...
    if (mysqli_num_rows($resultSolicitudesPendientes) > 0) {
        // Muestra la tabla de solicitudes pendientes
        echo "<table border='1'>";
        echo "<tr><th>Nombre</th><th>Correo</th></tr>";
    
        while ($row = mysqli_fetch_assoc($resultSolicitudesPendientes)) {
            echo "<tr>";
            echo "<td>" . $row["nombre"] . "</td>";
            echo "<td>" . $row["correo"] . "</td>";
    
            echo "<td><form method='post' action='aceptar_solicitud.php'>";
            echo "<input type='hidden' name='id_emisor' value='$id_usuario_actual'>";
            echo "<input type='hidden' name='id_receptor' value='" . $row["id_user"] . "'>";
            echo "<input type='submit' value='Aceptar Amigo'>";
            echo "</form></td>";

            // Agregar un segundo formulario para denegar la solicitud
            echo "<td><form method='post' action='denegar_solicitud.php'>";
            echo "<input type='hidden' name='id_emisor' value='$id_usuario_actual'>";
            echo "<input type='hidden' name='id_receptor' value='" . $row["id_user"] . "'>";
            echo "<input type='submit' name='denegar_amigo' value='Denegar Amigo'>";
            echo "</form></td>";
            echo "</tr>";
        }
        ...
        // Muestra un mensaje si no hay solicitudes pendientes
